...
def format_packet(source_id: int, dest_id: int, content: bytes) -> bytes:
    ...
    packet = f'{source_id} {dest_id}\n'.encode('ascii') + content
    #print(f"Packet Size new: {f'{source_id} {dest_id}\n'.encode('ascii')} bytes")  # Print the total size of the packet
    return packet
...
class Monitor:
	...
	def recv_end(self, recvfile, sender_id):
		"""Signals the end of receiving the file. Should be called after the last ACK is sent."""
		match = True

		if not os.path.exists(recvfile):
			match = False
		else:
			with open(self.file, 'rb') as orig:  # Open original file in binary mode
				with open(recvfile, 'rb') as recv:  # Open received file in binary mode
					orig_data = orig.read()
					recv_data = recv.read()

					if orig_data != recv_data:
						log(self.LOG_FILE_PATH, f'Received file does not match the original.')
						match = False

		log(self.LOG_FILE_PATH, f'File transmission correct: {match}')
		log(self.LOG_FILE_PATH, f'Number of Packets Received: {self.out_packets.get(sender_id, 0)}')
		log(self.LOG_FILE_PATH, f'Total Bytes Transmitted: {self.in_data.get(sender_id, 0)}')
		log(self.LOG_FILE_PATH, f'Total Time: {round(self.total_time, 2)} secs')
